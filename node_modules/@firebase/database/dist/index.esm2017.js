...
function repoGetValue(repo, query) {
    ...
    return repo.server_.get(query).then(payload => {
        const node = nodeFromJSON$1(payload).withIndex(query._queryParams.getIndex());
...
class DataSnapshot$1 {
    ...
    child(path) {
        ...
        return new DataSnapshot$1(this._database, this._delegate.child(path));
    }
    /**
    ...
    forEach(action) {
        ...
        return this._delegate.forEach(expDataSnapshot => action(new DataSnapshot$1(this._database, expDataSnapshot)));
    }
    /**
...
class Query {
    ...
    on(eventType, callback, cancelCallbackOrContext, context) {
        ...
        const valueCallback = (expSnapshot, previousChildName) => {
            callback.call(ret.context, new DataSnapshot$1(this.database, expSnapshot), previousChildName);
        };
    ...
    get() {
        return get(this._delegate).then(expSnapshot => {
            return new DataSnapshot$1(this.database, expSnapshot);
        });
    }
    /**
    ...
    once(eventType, callback, failureCallbackOrContext, context) {
        ...
        const deferred = new Deferred();
        const valueCallback = (expSnapshot, previousChildName) => {
            const result = new DataSnapshot$1(this.database, expSnapshot);
            if (callback) {
...
class Reference extends Query {
    ...
    transaction(transactionUpdate, onComplete, applyLocally) {
        ...
        const result = runTransaction(this._delegate, transactionUpdate, {
            applyLocally
        }).then(transactionResult => new TransactionResult$1(transactionResult.committed, new DataSnapshot$1(this.database, transactionResult.snapshot)));
        if (onComplete) {
...
function registerDatabase(instance) {
    ...
    const namespace = instance.INTERNAL.registerComponent(new Component('database', (container, { instanceIdentifier: url }) => {
        /* Dependencies */
