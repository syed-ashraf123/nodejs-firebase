...
import { getUA, createMockUserToken, getModularInstance } from '@firebase/util';
        ...
        if (!this.db) {
            ...
            this.db = await new Promise((resolve, reject) => {
                ...
                request.onupgradeneeded = (event) => {
                    logDebug(LOG_TAG, 'Database "' + this.name + '" requires upgrade from version:', event.oldVersion);
                    const db = event.target.result;
                    this.schemaConverter
                        .createOrUpgrade(db, request.transaction, event.oldVersion, this.version)
                        .next(() => {
                        logDebug(LOG_TAG, 'Database upgrade to version ' + this.version + ' complete');
                    });
                };
            });
        }
    ...
    async runTransaction(action, mode, objectStores, transactionFn) {
        ...
        while (true) {
            ...
            try {
                ...
                logDebug(LOG_TAG, 'Transaction failed with error:', error.message, 'Retrying:', retryable);
                this.close();
                if (!retryable) {
...
const LOG_TAG$1 = 'WatchChangeAggregator';
/**
 ...
 * limitations under the License.
 */
const LOG_TAG$2 = 'LruGarbageCollector';
        ...
        this.gcTask = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection" /* LruGarbageCollection */, delay, async () => {
            ...
            try {
                ...
                if (isIndexedDbTransactionError(e)) {
                    logDebug(LOG_TAG$2, 'Ignoring IndexedDB error during garbage collection: ', e);
                }
                else {
 ...
 * limitations under the License.
 */
const LOG_TAG$3 = 'IndexedDbPersistence';
/**
    ...
    forceOwningTab) {
        ...
        if (this.window && this.window.localStorage) {
            ...
            if (forceOwningTab === false) {
                logError(LOG_TAG$3, 'LocalStorage is unavailable. As a result, persistence may not work ' +
...
async function localStoreReleaseTarget(localStore, targetId, keepPersistedTargetData) {
    ...
    try {
        ...
        if (isIndexedDbTransactionError(e)) {
            ...
            logDebug(LOG_TAG$4, `Failed to update sequence numbers for target ${targetId}: ${e}`);
        }
        else {
            throw e;
        }
 ...
 * limitations under the License.
 */
const LOG_TAG$5 = 'MemoryPersistence';
/**
 ...
 * limitations under the License.
 */
const LOG_TAG$6 = 'SharedClientState';
/**
 ...
 * limitations under the License.
 */
const LOG_TAG$9 = 'PersistentStream';
 ...
 * limitations under the License.
 */
const LOG_TAG$a = 'OnlineStateTracker';
...
async function syncEngineRegisterPendingWritesCallback(syncEngine, callback) {
    ...
    if (!canUseNetwork(syncEngineImpl.remoteStore)) {
        logDebug(LOG_TAG$d, 'The network is disabled. The task returned by ' +
...
async function loadBundleImpl(syncEngine, reader, task) {
    try {
        ...
        logWarn(LOG_TAG$d, `Loading bundle failed with ${e}`);
        task._failWith(e);
    }
}
 ...
 * limitations under the License.
 */
const LOG_TAG$e = 'FirestoreClient';
 ...
 * limitations under the License.
 */
const LOG_TAG$f = 'ComponentProvider';
/**
...
class AsyncQueueImpl {
    ...
    async retryNextOp() {
        ...
        try {
            ...
            if (isIndexedDbTransactionError(e)) {
                logDebug(LOG_TAG$g, 'Operation failed with retryable error: ' + e);
            }
            else {
...
function registerFirestore(variant) {
    ...
    _registerComponent(new Component('firestore-exp', (container, { options: settings }) => {
        ...
        const firestoreInstance = new FirebaseFirestore$1(app, container.getProvider('auth-internal'));
